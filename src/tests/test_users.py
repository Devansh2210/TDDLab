...
def test_update_user(test_app, test_database, add_user):
    user = add_user('Devansh', 'devansh@algonquincollege.com')
    client = test_app.test_client()
    resp = client.put(
        f'/users/{user.id}',
        data=json.dumps({
    ...
    data = json.loads(resp.data.decode())
    assert resp.status_code == 200
    assert 'The email of the user updated to devansh@gmail.com. The password of the user updated to Devansh2' in data.get('message')
